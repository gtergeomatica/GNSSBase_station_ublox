...
def main():
    ...
    for i in os.listdir('{}/temporary'.format(os.path.dirname(os.path.realpath(__file__)))):
        out_raw=i[:-4]
        start_time=i[13:24]
        time_instant=datetime.strptime(i[13:24],'%Y%j%H%M%S')
        year=time_instant.strftime('%Y')
        months=time_instant.strftime('%b')
        folder_name_day='%04s-%02s-%02s'%(time_instant.strftime('%Y'),time_instant.strftime('%m'),time_instant.strftime('%d'))

        print(out_raw,start_time,remote_folder,year,months,folder_name_day)
        #1569,raw_obs_LIGE_20203370900,20203370900,/www.gter.it/stazione_gnss_ufficio/dati_rinex_orari/,2020,Dec,2020-12-02
